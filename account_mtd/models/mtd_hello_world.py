...
class MtdHelloWorld(models.Model):
    ...
    def action_hello_world_connection(self):
        if self.name == "Hello World":
            ...
            self._logger.info("Connection button Clicked - endpoint name {}, redirect URL:- {}, Path url:- {}".format(self.name, self.redirect, self.path))
            ...
            if not token_record.access_token and not token_record.refresh_token:
                ...
                self._logger.info("Connection button Clicked - endpoint name {}, Checking to see if a request is in process".format(self.name))
                if record:
...
""".format(datetime.now().date(), datetime.now().time(), hmrc_connection_url, req.status_code, response_token['message'])
            ...
            if record_id:
                ...
                self._logger.info("-------Redirect is:- /web#id={}&view_type=form&model=mtd.hello_world&menu_id={}&action={}".format(record_id, menu_id, action))
            ...
            self._logger.info("_json_command - code 401 found, user button clicked,  and message was:- {} ".format(response_token['message']))
...
{}
    ...
    def get_user_authorisation(self):
        ...
        response_token = json.loads(req.text)
        self._logger.info("(Step 1) Get authorisation - received response of the request:- {}, and its text:- {}".format(req, response_token))
        if req.ok:
...
""".format(datetime.now().date(), datetime.now().time(), authorisation_url, req.status_code, response_token=['message'])
    ...
    def exchange_user_authorisation(self, auth_code, record_id, tracker_id):
        ...
        self._logger.info("(Step 2) exchange authorisation code - Data which will be asend in the request:-  {}".format(json.dumps(data_user_info)))
        headers = {
            'Content-Type': 'application/json',
            }
        self._logger.info("(Step 2) exchange authorisation code - headers which will be asend in the request:-  {}".format(headers))
        req = requests.post(token_location_uri, data=json.dumps(data_user_info), headers=headers)
        response_token = json.loads(req.text)
        self._logger.info("(Step 2) exchange authorisation code - received response of the request:- {}, and its text:- {}".format(req, response_token))
        if req.ok:
            ...
            self._logger.info("(Step 2) exchange authorisation code - api_token table id where info is stored:- {}".format(api_token))
...
{}
            ...
            self._logger.info("(Step 2) exchange authorisation code - log:- {}".format(construct_text))
            # action_id
            action = self.env.ref('account_mtd.action_mtd_hello_world')
            # menu_id
            menu_id = self.env.ref('account_mtd.submenu_mtd_hello_world')
            self._logger.info("(Step 2) exchange authorisation code - redirect URI :- /web#id={}&view_type=form&model=mtd.hello_world&menu_id=72&action={}".format(record_id, menu_id, action))
    ...
    def refresh_user_authorisation(self, token_record=None):
        ...
        self._logger.info("(Step 4) refresh_user_authorisation - hmrc authorisation url:- {}".format(hmrc_authorisation_url))
        ...
        self._logger.info("(Step 4) refresh_user_authorisation - data to send in request:- {}").format(data_user_info)
        self._logger.info("(Step 4) refresh_user_authorisation - headers to send in request:- {}").format(headers)
        req = requests.post(hmrc_authorisation_url, data=json.dumps(data_user_info), headers=headers)
        response_token = json.loads(req.text)
        self._logger.info("(Step 4) refresh_user_authorisation - received response of the request:- {}, and its text:- {}".format(req, response_token))
        if req.ok:
            api_token.access_token = response_token['access_token']
            api_token.refresh_token = response_token['refresh_token']
            api_token.expires_in = json.dumps(response_token['expires_in'])
            ...
            self._logger.info("(Step 4) refresh_user_authorisation - error 400, Need to obtain new access code and refresh token")
...
{}
            ...
            self._logger.info("(Step 4) refresh_user_authorisation - other error:- {}".format(construct_text))
